<x-admin.layout>
    <div class="mb-6">
        <a href="{{ route('admin.locations.bingo-items.index', $bingoItem->location) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sky-600 hover:text-sky-700 bg-sky-50 hover:bg-sky-100 border border-sky-100 rounded-md transition-colors text-sm font-medium shadow-sm">
    ...
    <div class="bg-pure-white overflow-hidden rounded-card shadow-card p-6">
        ...
        <form method="POST" action="{{ route('admin.bingo-items.update', $bingoItem) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea id="fact" name="fact" rows="3" class="mt-1 block w-full border-surface-medium bg-pure-white text-deep-black focus:border-action focus:ring-action rounded-input shadow-sm">{{ old('fact', $bingoItem->fact) }}</textarea>
