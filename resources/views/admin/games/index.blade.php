            ...
            <div class="bg-pure-white rounded-card shadow-card p-4 flex justify-between items-center">
                <div>
                    <div class="flex items-center gap-3">
                        <span class="font-mono text-lg font-bold text-deep-black">{{ $game->pin }}</span>
                        @switch($game->status)
                            @case('lobby')
                                <span class="px-2 py-0.5 text-xs font-semibold rounded-badge bg-action-100 text-action-700">
                                    Lobby
                                </span>
                                @break
                            @case('started')
                                <span class="px-2 py-0.5 text-xs font-semibold rounded-badge bg-forest-100 text-forest-700">
                                    Gestart
                                </span>
                                @break
                            @case('finished')
                                <span class="px-2 py-0.5 text-xs font-semibold rounded-badge bg-surface-medium text-deep-black">
                                    Afgelopen
                                </span>
                                @break
                        @endswitch
                    </div>
                    <p class="mt-1 text-sm text-forest-600">{{ $game->location->name ?? 'Onbekend' }}</p>
                    <p class="mt-1 text-xs text-forest-500">{{ $game->created_at->format('d-m-Y H:i') }}</p>
                </div>
