<x-admin.layout>
    <div class="mb-6">
        <a href="{{ route('admin.locations.route-stops.index', $location) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sky-600 hover:text-sky-700 bg-sky-50 hover:bg-sky-100 border border-sky-100 rounded-md transition-colors text-sm font-medium shadow-sm">
        ...
        <form method="POST" action="{{ route('admin.locations.route-stops.store', $location) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <x-text-input id="sequence" name="sequence" type="number" min="0" class="mt-1 block w-full" :value="old('sequence', $nextSequence)" required />
            ...
            <div class="mb-4">
                ...
                <textarea id="question_text" name="question_text" rows="3" class="mt-1 block w-full border-surface-medium bg-pure-white text-deep-black focus:border-action focus:ring-action rounded-input shadow-sm" required>{{ old('question_text') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <x-text-input id="option_a" name="option_a" type="text" class="mt-1 block w-full" :value="old('option_a')" required />
                    <x-input-error :messages="$errors->get('option_a')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="option_b" value="Antwoord B" />
                    <x-text-input id="option_b" name="option_b" type="text" class="mt-1 block w-full" :value="old('option_b')" required />
                    <x-input-error :messages="$errors->get('option_b')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="option_c" value="Antwoord C (optioneel)" />
                    <x-text-input id="option_c" name="option_c" type="text" class="mt-1 block w-full" :value="old('option_c')" />
                    <x-input-error :messages="$errors->get('option_c')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="option_d" value="Antwoord D (optioneel)" />
                    <x-text-input id="option_d" name="option_d" type="text" class="mt-1 block w-full" :value="old('option_d')" />
            ...
            <div class="mb-6">
                <x-input-label for="image" value="Vraag afbeelding (optioneel)" />
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.locations.route-stops.index', $location) }}">
