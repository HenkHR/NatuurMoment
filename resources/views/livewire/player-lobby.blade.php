<div wire:poll.2s.visible="checkGameStatus" class="bg-surface-light min-h-screen flex flex-col">

    <header
        class="bg-forest-700 text-pure-white shadow-card"
        style="clip-path: polygon(0 0, 100% 0, 100% calc(100% - 28px), 0 100%);"
    >
        <div class="max-w-5xl mx-auto px-4 lg:px-8 pt-8 pb-10 text-center">
            <h1 class="text-3xl md:text-4xl font-bold">Game Lobby</h1>
            <p class="text-base md:text-lg font-semibold mt-1">
                {{ $locationName }}
            </p>
        </div>
    </header>

    <main class="flex-1">
        <div class="max-w-5xl mx-auto px-4 lg:px-8 pt-6 pb-16">

            <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-4 md:mb-6">
                <div
                    class="w-full sm:w-auto min-w-[180px] max-w-xs
                           bg-sky-500 text-pure-white text-sm font-semibold
                           rounded-button px-6 py-2 shadow-card
                           flex items-center justify-center
                           focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2
                           focus-visible:ring-sky-500 focus-visible:ring-offset-surface-light"
                    tabindex="0"
                    aria-label="Spelcode {{ $pin }}"
                >
                    Code: {{ $pin }}
                </div>

                <button
                    type="button"
                    class="w-full sm:w-auto min-w-[180px] max-w-xs
                           bg-forest-500 text-pure-white text-sm font-semibold
                           rounded-button px-6 py-2 shadow-card
                           flex items-center justify-center
                           focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2
                           focus-visible:ring-forest-500 focus-visible:ring-offset-surface-light"
                >
                    Speluitleg
                </button>
            </div>

            <section class="bg-pure-white shadow-card rounded-card overflow-hidden">
                <div class="px-4 pt-4 pb-5">
                    <p class="text-sm font-semibold text-deep-black mb-3">
                        Aantal spelers: {{ $playerCount }}
                    </p>

                    @if(count($players))
                        <ul class="space-y-3">
                            @foreach($players as $player)
                                <li
                                    wire:key="player-{{ $player['id'] }}"
                                    class="flex items-center justify-between bg-surface-medium rounded-card px-4 py-3 shadow-card"
                                >
                                    <span class="font-semibold text-deep-black">
                                        {{ $player['name'] }}
                                    </span>

                                    @if($player['name'] === $playerName)
                                        <span class="text-xs font-semibold text-forest-700">
                                            (jij)
                                        </span>
                                    @endif
                                </li>
                            @endforeach
                        </ul>
                    @else
                        <p class="text-sm text-deep-black/70 py-4">
                            Nog geen spelers...
                        </p>
                    @endif
                </div>
            </section>
        </div>
    </main>

    <footer class="bg-sky-500 py-6 mt-auto">
        <div class="max-w-5xl mx-auto px-4 lg:px-8">
            <div class="bg-pure-white text-sky-700 font-semibold text-sm md:text-base px-6 py-3 rounded-card shadow-card text-center">
                Wachten tot spel is gestart...
            </div>
        </div>
    </footer>
</div>
